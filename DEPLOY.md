...
なりすましてログインされます。顧客情報とアポ内容が入っているため、社内利用でも必須です。
...
`/data`（Docker）または `/var/lib/ocean-ai`（systemd）に顧客情報・アポ履歴・アカウントが入ります。
**ここだけバックアップすれば復旧できます。**
...
この章のJSONファイル保存（`lib/store-file.ts`）は使えません。顧客情報もアカウントも
