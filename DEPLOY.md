...
インターネット / 社内LAN
  ...
  └───────┬───────┘
          │
          ▼
   /data （クライアント情報・アポ履歴・アカウント）※バックアップ対象
```
...
なりすましてログインされます。クライアント情報とアポ内容が入っているため、社内利用でも必須です。
...
`/data`（Docker）または `/var/lib/ocean-ai`（systemd）にクライアント情報・アポ履歴・アカウントが入ります。
**ここだけバックアップすれば復旧できます。**
...
バックアップファイルにはクライアント情報が含まれます。保存先の権限（`chmod 600`）と保管場所に注意してください。
...
サーバーレス環境（Vercel、Netlifyなど）は実行環境のファイルシステムが揮発するため、
この章のJSONファイル保存（`lib/store-file.ts`）は使えません。クライアント情報もアカウントも
次のリクエストで消えます。永続ボリュームを持てるコンテナ環境（Fly.io、Render、
社内のDockerホストなど）であれば、この章の手順のままで動きます。

**Netlifyについては対応済みです。** Netlify Blobs（Netlifyのキーバリューストア）を使う
永続化バックエンド（`lib/store-blobs.ts`）を用意しており、Netlify上では自動的にそちらへ
切り替わります。手順は [NETLIFY.md](./NETLIFY.md) を参照してください。
Vercelなど他のサーバーレス環境で使う場合は、先に上記のDB移行（PostgreSQL等）を済ませてください。
